...
* enumerate detected Logitech-compatible devices from `solaar show`
* optionally run `solaar config` against a selected detected device (or any custom device name/ID)
* optionally run `libinput debug-events` and `keyd monitor`
