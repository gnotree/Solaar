...
list_detected_devices() {
    local show_output
    if ! show_output="$($SOLAAR_BIN show 2>/dev/null)"; then
        warn "Unable to enumerate devices from 'solaar show'."
        return 1
    fi

    mapfile -t DETECTED_DEVICES < <(
        awk 'match($0, /^[[:space:]]*[0-9]+:[[:space:]]*(.+)$/, m) {
            name = m[1]
            if (name !~ /[Rr]eceiver/) print name
        }' <<<"$show_output"
    )

    if [[ "${#DETECTED_DEVICES[@]}" -eq 0 ]]; then
        warn "No compatible Logitech devices were parsed from 'solaar show'."
        return 1
    fi

    say "Detected Logitech devices:"
    local i
    for i in "${!DETECTED_DEVICES[@]}"; do
        printf '  [%d] %s\n' "$((i + 1))" "${DETECTED_DEVICES[$i]}"
    done
    return 0
}

if ask_yes_no "Enumerate detected Logitech devices now?" y; then
    list_detected_devices || true
fi

if ask_yes_no "Run 'solaar config <device>' now?" n; then
    DEVICE_NAME=""

    if list_detected_devices; then
        DEVICE_SELECTION="$(ask_value "Select device number or enter a custom device name" "1")"
        if [[ "$DEVICE_SELECTION" =~ ^[0-9]+$ ]] && (( DEVICE_SELECTION >= 1 )) && (( DEVICE_SELECTION <= ${#DETECTED_DEVICES[@]} )); then
            DEVICE_NAME="${DETECTED_DEVICES[$((DEVICE_SELECTION - 1))]}"
        else
            DEVICE_NAME="$DEVICE_SELECTION"
        fi
    else
        DEVICE_NAME="$(ask_value "Device name, serial, or numeric device id" "")"
    fi

    if [[ -z "$DEVICE_NAME" ]]; then
        warn "No device selected; skipping 'solaar config'."
    else
